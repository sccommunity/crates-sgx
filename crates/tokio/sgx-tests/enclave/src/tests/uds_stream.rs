...
async fn accept_read_write() {
    assert_ok!(accept_read_write_inner().await);
}

async fn accept_read_write_inner() -> std::io::Result<()> {
...
#[crates_unittest::test]
async fn shutdown() {
    assert_ok!(shutdown_inner().await);
}

async fn shutdown_inner() -> std::io::Result<()> {
    let dir = tempfile::Builder::new()
        .prefix("tokio-uds-tests")
        .tempdir()
        .unwrap();
