...
fn works_2() {
    ...
    a_tx.send(33).unwrap();
    b_tx.send(33).unwrap();
    assert!(spawn.poll_stream_notify(&notify_noop(), 0).unwrap().is_ready());
    c_tx.send(33).unwrap();
    assert!(spawn.poll_stream_notify(&notify_noop(), 0).unwrap().is_ready());
}
...
fn finished_future_ok() {
    ...
    let mut spawn = futures::executor::spawn(stream);
    for _ in 0..10 {
        assert!(spawn.poll_stream_notify(&notify_noop(), 0).unwrap().is_not_ready());
    }

    b_tx.send(Box::new(())).unwrap();
    let next = spawn.poll_stream_notify(&notify_noop(), 0).unwrap();
